def calc_saving(income, goal, time, balance=0, expenses=0):
    ...
    goal = float(goal)
    if income < expenses:
        overspend = '{0:,.2f}'.format(abs(round(income-expenses, 2)))
        return f'you are overspending by ${overspend}'
    else:
        ...
        if goal > 12 * income * time:
            ...
            save = round(goal/time/12, 2)
            allowance = '{0:,.2f}'.format(round(income - save, 2))
